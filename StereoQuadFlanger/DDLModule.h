...
public:
	...
	// delay line specific variables, based on Basic Delay equations
	// Delay line is created dynmaically and will be destroyed in destructor
	// We have to dynamically create and flush out the buffer each time prepareForPlay() is called
	float* m_pBuffer;
	int m_nReadIndex;
	int m_nWriteIndex;
	int m_nBufferSize;

	// these variables are for modifying the module to by used by a parent plug-in
	//
	// flag for enabling/disabling external feedback mode
	bool m_bUseExternalFeedback; 

	// user supplied feedback sample value
	bool m_fFeedbackIn;


	// cooking function to figuring out the delay time in samples from the delay time in milliseconds
	// this function converts variables into correct units
	void cookVariables();
