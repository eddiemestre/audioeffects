...
CModDelayModule::CModDelayModule()
{
	...
	m_LFO.m_uOscType = m_uLFOType;		// trialgne enum{tri, sin};
...
void CModDelayModule::updateLFO() {
	...
	m_LFO.m_uOscType = m_uLFOType == 0 ? 2 : 0;		// tri = 0 -> Oscillator's tri = 2 and sin = 1 -> Oscillator's sin = 0
...
// DDL function to set:
//						- the DDL Feedback amount (dsabled for Vibrato)
...
void CModDelayModule::cookModType() {
	...
	case Vibrato:
	{
		m_fMinDelay_mSec = 0;
		m_fMaxDelay_mSec = 7;
		m_DDL.m_f_WetLevel_pct = 100;
		m_DDL.m_f_Feedback_pct = 0.0;		// NOTE! no FB for vibrato
		break;
	}
	case Chorus:
	{
...
/*
	...
	NOTES: - the range for the flanger/vibrato is simply mapped from min
	to max starting at min: fLFOSample*(m_fMaxDelay_mSec - m_fMinDelay_mSec) +
	m_fMinDelay_mSec

	the range for the Chorus includes the starting offset fStart = m_fMinDelay_mSec +
	m_fChorusOffset;
*/
float CModDelayModule::calculateDelayOffset(float fLFOSample) {

	if (m_uModType == Flanger || m_uModType == Vibrato) {
		// flanger 0->1 gets maped to 0->maxdelay
...
bool __stdcall CModDelayModule::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	if (m_uLFOPhase == quad) {
		...
		m_LFO.m_bInvert = true;
		fDelay = calculateDelayOffset(fYn);		// inverted LFO
	}
	else {
...
2				m_fFeedback_pct                   10
3				m_fChorusOffset                   11
4				m_uModType                        41
5				m_uLFOType                        42
6				m_uLFOPhase                       43
