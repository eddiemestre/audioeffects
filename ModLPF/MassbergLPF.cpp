...
*/
CMassbergLPF::~CMassbergLPF(void)
{
...
*/
bool __stdcall CMassbergLPF::initialize()
{
	// Add your code here
...
// Function: calculateLPFCoeffs
// Purpose: calculates the coefficients of the MassbergLPF
// Paramters: CutoffFrequency and Q
// Returns: Nothing
void CMassbergLPF::calculateLPFCoeffs(float fCutoffFreq, float fQ) {

	// design equations
	float theta_c = 2.0 * pi * fCutoffFreq / (float)m_nSampleRate;

	float g1 = 2 / sqrt(pow(2 - pow((sqrt(2)*pi) / theta_c, 2), 2) + pow((2 * pi) / (fQ*theta_c), 2));
	...
	if (fQ > sqrt(5)) {
		...
		omegaR_L = theta_c * sqrt(1 - (1 / pow(2 * fQ, 2)));

		omegaR_U = tan(omegaR_L / 2);

		omegaS_U = omegaR_U * pow((pow(gR, 2) - pow(g1, 2)) / (pow(gR, 2) - 1), (1 / 4));
	}

	else {

		omegaM_L = theta_c * sqrt((2 - (1 / (2 * pow(fQ, 2))) + sqrt(((1 - (4 * pow(fQ, 2))) / (pow(fQ, 4))) + (4 / g1))) / 2);

		omegaM_U = tan(omegaM_L / 2);

		omegaS_U = theta_c * pow(1 - pow(g1, 2), 1 / 4) / 2;

		omegaS_U2 = fmin(omegaS_U, omegaM_U);
	}

	// Calculate the pole and zero frequencies (small omega), gains (g), and Qs

	float omegaP_L = 2 * atan(omegaS_U);
	float omegaZ_L = 2 * atan(omegaS_U / sqrt(g1));

	float gp = 1 / (sqrt(pow(1 - pow(omegaP_L / theta_c, 2), 2) + pow(omegaP_L / (fQ*theta_c), 2)));
	float gz = 1 / (sqrt(pow(1 - pow(omegaZ_L / theta_c, 2), 2) + pow(omegaZ_L / (fQ*theta_c), 2)));
	
	float Qp = sqrt((g1*(pow(gp, 2) - pow(gz, 2))) / ((g1 + pow(gz, 2)) * pow(g1 - 1, 2)));
	float Qz = sqrt((pow(g1, 2)*(pow(gp, 2) - pow(gz, 2))) / (pow(gz, 2)*(g1 + pow(gp, 2))*pow(g1 - 1, 2)));

	float gamma0 = pow(omegaS_U, 2) + ((1 / Qp) * omegaS_U) + 1;

	float alpa0 = pow(omegaS_U, 2) + ((sqrt(g1) / Qz) * omegaS_U) + g1;
	float alpa1 = 2 * (pow(omegaS_U, 2) - g1);
	float alpa2 = pow(omegaS_U, 2) - ((sqrt(g1) / Qz) * omegaS_U) + g1;

	float beta1 = 2 * (pow(omegaS_U, 2) - 1);
	float beta2 = pow(omegaS_U, 2) - ((1 / Qp) * omegaS_U) + 1;

	//left channel:

	m_LeftLPF.m_f_a0 = alpa0 / gamma0;
	m_LeftLPF.m_f_a1 = alpa1 / gamma0;
	m_LeftLPF.m_f_a2 = alpa2 / gamma0;
	m_LeftLPF.m_f_b1 = beta1 / gamma0;
	m_LeftLPF.m_f_b2 = beta2 / gamma0;

	//left channel:

	m_RightLPF.m_f_a0 = alpa0 / gamma0;
	m_RightLPF.m_f_a1 = alpa1 / gamma0;
	m_RightLPF.m_f_a2 = alpa2 / gamma0;
	m_RightLPF.m_f_b1 = beta1 / gamma0;
	m_RightLPF.m_f_b2 = beta2 / gamma0;
...
processRackAFXMessage()
	...
	See the website www.willpirkle.com for more details
*/
void __stdcall CMassbergLPF::processRackAFXMessage(UINT uMessage, PROCESS_INFO& processInfo)
{
...
*/
bool __stdcall CMassbergLPF::prepareForPlay()
{
	// Add your code here:
...
*/
bool __stdcall CMassbergLPF::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
...
//
bool __stdcall CMassbergLPF::userInterfaceChange(int nControlIndex)
{
...
bool __stdcall CMassbergLPF::checkUpdateGUI(int nControlIndex, float fValue, CLinkedList<GUI_PARAMETER>& guiParameters, bool bLoadingPreset)
{
...
bool __stdcall CMassbergLPF::processAuxInputBus(audioProcessData* pAudioProcessData)
{
...
*/
bool __stdcall CMassbergLPF::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
/* processAudioBuffer
	...
	******************************
	...
	m_HostProcessInfo.uTimeSigDenomintor = Host Time Signature Denominator (if supported by host)
*/
bool __stdcall CMassbergLPF::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CMassbergLPF::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
bool __stdcall CMassbergLPF::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CMassbergLPF::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CMassbergLPF::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CMassbergLPF::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CMassbergLPF::midiClock()
{
...
bool __stdcall CMassbergLPF::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
*/
void CMassbergLPF::doVSTSampleAccurateParamUpdates()
{
...
void* __stdcall CMassbergLPF::showGUI(void* pInfo)
{
...
bool __stdcall CMassbergLPF::initUI()
{
	// ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! -------------------------------------- //
