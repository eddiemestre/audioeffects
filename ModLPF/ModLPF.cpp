...
*/
bool __stdcall CModLPF::initialize()
{
	// Add your code here
...
// updateModules()
//
void CModLPF::updateModules() {
	m_ModLeft.m_f_Fc_Hz = m_f_fc_Hz;
	m_ModRight.m_f_Fc_Hz = m_f_fc_Hz;

	m_ModLeft.m_f_Q = m_f_Q;
	m_ModRight.m_f_Q = m_f_Q;

	// update LFOs
	m_LFO.m_fFrequency_Hz = m_fModFrequency_Hz;
	m_LFO.m_uOscType = m_uLFOType;

	//m_fModDepth = m_f_fc_Hz;

	m_LFO.cookFrequency();

	// min/max Fc for ModDepth
	m_fMaxFc = m_f_fc_Hz;
	m_fMinFc = 100.0;
}
...
// update LFO()
//
float CModLPF::calculateDepthMod(float fLFOSample) {
	return (m_fModDepth_pct / 100.0) * (fLFOSample * (m_fMaxFc - m_fMinFc)) + m_fMinFc;
}
...
processRackAFXMessage()
	...
	See the website www.willpirkle.com for more details
*/
void __stdcall CModLPF::processRackAFXMessage(UINT uMessage, PROCESS_INFO& processInfo)
{
...
*/
bool __stdcall CModLPF::prepareForPlay()
{
	// Add your code here:
	m_ModLeft.prepareForPlay();
	m_ModRight.prepareForPlay();
...
*/
bool __stdcall CModLPF::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	// get LFO values, just normal for now
	float fYn = 0;
	float fYqn = 0;
	...
	// calculate modDepth
	float fModDepth = 0.0;
	fModDepth = calculateDepthMod(fYn);

	m_ModLeft.calculateLPFCoeffs(fModDepth, m_f_Q);
	m_ModRight.calculateLPFCoeffs(fModDepth, m_f_Q);

	m_ModLeft.processAudioFrame(&pInputBuffer[0], &pOutputBuffer[0], 1, 1);
	...
	if (uNumInputChannels == 2 && uNumOutputChannels == 2)
		m_ModRight.processAudioFrame(&pInputBuffer[1], &pOutputBuffer[1], 1, 1);
...
0				m_f_fc_Hz                         0
1				m_f_Q                             1
2				m_fModDepth_pct                   10
3				m_fModFrequency_Hz                11
4				m_uLFOType                        41
...
//
bool __stdcall CModLPF::userInterfaceChange(int nControlIndex)
{
	// decode the control index, or delete the switch and use brute force calls
	updateModules();
	m_ModLeft.calculateLPFCoeffs(m_f_fc_Hz, m_f_Q);
	m_ModRight.calculateLPFCoeffs(m_f_fc_Hz, m_f_Q);
	m_LFO.m_fFrequency_Hz = m_fModFrequency_Hz;
	m_LFO.m_uOscType = m_uLFOType;
...
bool __stdcall CModLPF::checkUpdateGUI(int nControlIndex, float fValue, CLinkedList<GUI_PARAMETER>& guiParameters, bool bLoadingPreset)
{
...
bool __stdcall CModLPF::processAuxInputBus(audioProcessData* pAudioProcessData)
{
...
*/
bool __stdcall CModLPF::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
/* processAudioBuffer
	...
	******************************
	...
	m_HostProcessInfo.uTimeSigDenomintor = Host Time Signature Denominator (if supported by host)
*/
bool __stdcall CModLPF::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CModLPF::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
bool __stdcall CModLPF::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CModLPF::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CModLPF::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CModLPF::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CModLPF::midiClock()
{
...
bool __stdcall CModLPF::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
*/
void CModLPF::doVSTSampleAccurateParamUpdates()
{
...
void* __stdcall CModLPF::showGUI(void* pInfo)
{
...
// **--0xDEA7--**
	...
	ui0->fUserDisplayDataHiLimit = 10000.000000;
	...
	ui2->m_pUserCookedFloatData = &m_fModDepth_pct;
	ui2->m_pUserCookedDoubleData = NULL;
	ui2->m_pUserCookedUINTData = NULL;
	ui2->cControlUnits = "%";
	ui2->cVariableName = "m_fModDepth_pct";
	...
	ui3->fInitUserFloatValue = 0.180000;
	ui3->fInitUserDoubleValue = 0;
	ui3->fInitUserUINTValue = 0;
	ui3->m_pUserCookedIntData = NULL;
	ui3->m_pUserCookedFloatData = &m_fModFrequency_Hz;
	ui3->m_pUserCookedDoubleData = NULL;
	ui3->m_pUserCookedUINTData = NULL;
	ui3->cControlUnits = "Hz";
	ui3->cVariableName = "m_fModFrequency_Hz";
	ui3->cEnumeratedList = "SEL1,SEL2,SEL3";
	ui3->dPresetData[0] = 0.180000;ui3->dPresetData[1] = 0.000000;ui3->dPresetData[2] = 0.000000;ui3->dPresetData[3] = 0.000000;ui3->dPresetData[4] = 0.000000;ui3->dPresetData[5] = 0.000000;ui3->dPresetData[6] = 0.000000;ui3->dPresetData[7] = 0.000000;ui3->dPresetData[8] = 0.000000;ui3->dPresetData[9] = 0.000000;ui3->dPresetData[10] = 0.000000;ui3->dPresetData[11] = 0.000000;ui3->dPresetData[12] = 0.000000;ui3->dPresetData[13] = 0.000000;ui3->dPresetData[14] = 0.000000;ui3->dPresetData[15] = 0.000000;
	ui3->cControlName = "Rate";
	...
	ui4->fUserDisplayDataHiLimit = 3.000000;
	...
	ui4->m_pUserCookedUINTData = &m_uLFOType;
	ui4->cControlUnits = "";
	ui4->cVariableName = "m_uLFOType";
	ui4->cEnumeratedList = "sine,saw,tri,square";
