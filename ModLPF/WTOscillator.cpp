...
CWTOscillator::CWTOscillator()
{
	...
	for (int i = 0; i < 1024; i++) {
		...
		if (i < 256) {
			...
			m_TriangleArray[i] = mtf2 * (i - 256) + btf2;	// mx_b; falling edge
		}
		else {
		...
		for (int g = 1; g <= 6; g++) {
			double n = double(g);
			m_SawtoothArray_BL5[i] += pow((float)-1.0, (float)(g + 1)) * (1.0 / n) * 
				sin(2.0 * pi * i * n / 1024.0);
		}		

		// triangle: += (-1)^g(1/2g+1^2)*sin(w(2g+1)T)
		for (int g = 0; g <= 3; g++) {
			double n = double(g);
			m_TriangleArray_BL5[i] += pow((float)-1.0, (float)n) * (1.0 / pow((float)(2 * n + 1), 
				(float)2.0)) * sin(2.0 * pi * (2.0 *n + 1) * i / 1024.0);
		}

		// square: += 1/2g+1 * sin(wnT)
		for (int g = 1; g <= 5; g += 2) {
			double n = double(g);
...
// doOscillate()
//
// Function: doOscillate()
// Purpose: does oscillator processing
// Parameters: two floats for pYn and pYqn
// Returns nothing
void CWTOscillator::doOscillate(float* pYn, float* pYqn) {
	...
	// set up second index for interpolation; wrap the buffer if needed
	int nReadIndexNext = nReadIndex + 1 > 1023 ? 0 : nReadIndex + 1;
	int nQuadReadIndexNext = nQuadPhaseReadIndex + 1 > 1023 ? 0 : nQuadPhaseReadIndex + 1;

	// interpolate the output (x1, x2, y1, y2, frac) - notice the way we set x1 and x2
	...
	case sine:
		...
		fQuadPhaseOutSample = dLinTerp(0, 1, m_SinArray[nQuadPhaseReadIndex], m_SinArray[nQuadReadIndexNext], fFrac);
		break;
	case saw:
		if (m_uTableMode == normal) {
			fOutSample = dLinTerp(0, 1, m_SawtoothArray[nReadIndex], m_SawtoothArray[nReadIndexNext], fFrac);
			fQuadPhaseOutSample = dLinTerp(0, 1, m_SawtoothArray[nQuadPhaseReadIndex], m_SawtoothArray[nQuadReadIndexNext], fFrac);
		}
		else {
			fOutSample = dLinTerp(0, 1, m_SawtoothArray_BL5[nReadIndex], m_SawtoothArray_BL5[nReadIndexNext], fFrac);
			fQuadPhaseOutSample = dLinTerp(0, 1, m_SawtoothArray_BL5[nQuadPhaseReadIndex], m_SawtoothArray_BL5[nQuadReadIndexNext], fFrac);
		}
		break;
	case tri:
		if (m_uTableMode == normal) {
			fOutSample = dLinTerp(0, 1, m_TriangleArray[nReadIndex], m_TriangleArray[nReadIndexNext], fFrac);
			fQuadPhaseOutSample = dLinTerp(0, 1, m_TriangleArray[nQuadPhaseReadIndex], m_TriangleArray[nQuadReadIndexNext], fFrac);
		}
		else {
			fOutSample = dLinTerp(0, 1, m_TriangleArray_BL5[nReadIndex], m_TriangleArray_BL5[nReadIndexNext], fFrac);
			fQuadPhaseOutSample = dLinTerp(0, 1, m_TriangleArray_BL5[nQuadPhaseReadIndex], m_TriangleArray_BL5[nQuadReadIndexNext], fFrac);
	...
	case square:
		if (m_uTableMode == normal) {
			...
			fQuadPhaseOutSample = dLinTerp(0, 1, m_SquareArray[nQuadPhaseReadIndex], m_SquareArray[nQuadReadIndexNext], fFrac);
		}
		else {
			fOutSample = dLinTerp(0, 1, m_SquareArray_BL5[nReadIndex], m_SquareArray_BL5[nReadIndexNext], fFrac);
			fQuadPhaseOutSample = dLinTerp(0, 1, m_SquareArray_BL5[nQuadPhaseReadIndex], m_SquareArray_BL5[nQuadReadIndexNext], fFrac);
		}
		break;
	default:
		fOutSample = dLinTerp(0, 1, m_SinArray[nReadIndex], m_SinArray[nReadIndexNext], fFrac);
	...
	// add the increment for next tme
...
// cookFrequency()
//
// Function: cookFrequency()
// Purpose:	cooks member variables
// Parameters:	nothing
// Returns: nothing
...
processRackAFXMessage()
	...
	See the website www.willpirkle.com for more details
*/
void __stdcall CWTOscillator::processRackAFXMessage(UINT uMessage, PROCESS_INFO& processInfo)
{
...
bool __stdcall CWTOscillator::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	// call the oscillator function, return values into fY and fYq
	doOscillate(&fY, &fYq);

	// write out
	pOutputBuffer[0] = fY * m_fVolume;

	// Mono-In, Stereo-Out (AUX Effect)
	if (uNumInputChannels == 1 && uNumOutputChannels == 2)
		pOutputBuffer[1] = fYq * m_fVolume;
...
bool __stdcall CWTOscillator::checkUpdateGUI(int nControlIndex, float fValue, CLinkedList<GUI_PARAMETER>& guiParameters, bool bLoadingPreset)
{
...
bool __stdcall CWTOscillator::processAuxInputBus(audioProcessData* pAudioProcessData)
{
...
*/
bool __stdcall CWTOscillator::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
*/
void CWTOscillator::doVSTSampleAccurateParamUpdates()
{
...
void* __stdcall CWTOscillator::showGUI(void* pInfo)
{
...
// **--0xDEA7--**
	...
	ui2->m_pUserCookedUINTData = &m_uOscType;
	ui2->cControlUnits = "";
	ui2->cVariableName = "m_uOscType";
	...
	ui3->m_pUserCookedUINTData = &m_uTableMode;
	ui3->cControlUnits = "";
	ui3->cVariableName = "m_uTableMode";
	ui3->cEnumeratedList = "normal,bandlimit";
	...
	ui4->m_pUserCookedUINTData = &m_uPolarity;
	ui4->cControlUnits = "";
	ui4->cVariableName = "m_uPolarity";
	ui4->cEnumeratedList = "bipolar,unipolar";
