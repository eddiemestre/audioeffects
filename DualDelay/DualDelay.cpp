...
CDualDelay::CDualDelay()
{
	...
	// set external feedbacks to false
	m_SD_Left.m_DDL_Left.m_bUseExternalFeedback = false;
	m_SD_Left.m_DDL_Right.m_bUseExternalFeedback = false;

	m_SD_Right.m_DDL_Left.m_bUseExternalFeedback = false;
	m_SD_Right.m_DDL_Right.m_bUseExternalFeedback = false;
	...
	// HPF Fc
	m_HP.m_f_Fc_Hz = m_f_fc_HPF;

	// LPF Fc
	// Massberg
	m_LP.m_f_Fc_Hz = m_f_fc_LPF;
	// Resonant LPF
	m_RLP.m_f_fc_Hz = m_f_fc_LPF;
}
...
*/
bool __stdcall CDualDelay::initialize()
{
...
//
// 
void CDualDelay::SetDelayVariables(bool bCook) {
	// forward variables to member objects
	//
	// Delay
	// left (Delay One)
	m_SD_Left.m_DDL_Left.m_fDelay_ms = m_fDelayOne;
	m_SD_Left.m_DDL_Right.m_fDelay_ms = m_fDelayOne;

	// right (Delay Two)
	m_SD_Right.m_DDL_Left.m_fDelay_ms = m_fDelayTwo;
	m_SD_Right.m_DDL_Right.m_fDelay_ms = m_fDelayTwo;

	// feedback
	m_SD_Left.m_DDL_Left.m_f_Feedback_pct = m_f_Feedback;
	m_SD_Left.m_DDL_Right.m_f_Feedback_pct = m_f_Feedback;

	m_SD_Right.m_DDL_Left.m_f_Feedback_pct = m_f_Feedback;
	m_SD_Right.m_DDL_Right.m_f_Feedback_pct = m_f_Feedback;

	// Wet/Dry Mix
	m_SD_Left.m_DDL_Left.m_f_WetLevel_pct = m_fWetLevel;
	m_SD_Left.m_DDL_Right.m_f_WetLevel_pct = m_fWetLevel;

	m_SD_Right.m_DDL_Left.m_f_WetLevel_pct = m_fWetLevel;
	m_SD_Right.m_DDL_Right.m_f_WetLevel_pct = m_fWetLevel;

	// cook if desired
	if (bCook)
		m_SD_Left.m_DDL_Left.cookVariables();
		m_SD_Left.m_DDL_Right.cookVariables();

		m_SD_Right.m_DDL_Left.cookVariables();
		m_SD_Right.m_DDL_Right.cookVariables();
}
...
processRackAFXMessage()
	...
	See the website www.willpirkle.com for more details
*/
void __stdcall CDualDelay::processRackAFXMessage(UINT uMessage, PROCESS_INFO& processInfo)
{
...
*/
bool __stdcall CDualDelay::prepareForPlay()
{
	// Add your code here:
	SetDelayVariables(false);
	m_SD_Left.m_DDL_Left.prepareForPlay();
	m_SD_Left.m_DDL_Right.prepareForPlay();

	m_SD_Right.m_DDL_Left.prepareForPlay();
	m_SD_Right.m_DDL_Right.prepareForPlay();

	m_HP.calculateHPFCoeffs(m_f_fc_HPF, Q);
	// Massberg LPF
	m_LP.calculateLPFCoeffs(m_f_fc_LPF, Q);
	// Resonant LPF
	m_RLP.calculateLPFCoeffs(m_f_fc_LPF, Q);
...
*/
bool __stdcall CDualDelay::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	float fRightOutOne = 0;
	float fRightOutTwo = 0;
	float fRightOutAll = 0;
	float fRightHP = 0;
	float fRightLP = 0;

	// get left and right inputs; note the set up for mono
	...
	// left
	m_SD_Left.m_DDL_Left.processAudioFrame(&fLeftIn, &fLeftOutOne, 1, 1);
	m_SD_Left.m_DDL_Right.processAudioFrame(&fLeftIn, &fLeftOutTwo, 1, 1);

	// right
	m_SD_Right.m_DDL_Left.processAudioFrame(&fRightIn, &fRightOutOne, 1, 1);
	m_SD_Right.m_DDL_Right.processAudioFrame(&fRightIn, &fRightOutTwo, 1, 1);

	fLeftOutAll = fLeftOutOne + fLeftOutTwo;
	fRightOutAll = fRightOutOne + fRightOutTwo;

	fLeftHP = m_HP.m_LeftHPF.doBiQuad(fLeftOutAll);
	// using Resonant LPF
	fLeftLP = m_RLP.m_LeftLPF.doBiQuad(fLeftHP);
	fRightHP = m_HP.m_RightHPF.doBiQuad(fRightOutAll);
	// using Resonant LPF
	fRightLP = m_RLP.m_RightLPF.doBiQuad(fRightHP);


	// sum to create Left Out
	pOutputBuffer[0] = fLeftLP * m_fVolume;
	...
	if (uNumInputChannels == 2 && uNumOutputChannels == 2)
		pOutputBuffer[1] = fRightLP * m_fVolume;
...
//
bool __stdcall CDualDelay::userInterfaceChange(int nControlIndex)
{
	// set and cook delay variables
	SetDelayVariables(true);
	m_HP.calculateHPFCoeffs(m_f_fc_HPF, Q);
	// Massberg
	m_LP.calculateLPFCoeffs(m_f_fc_LPF, Q);
...
bool __stdcall CDualDelay::checkUpdateGUI(int nControlIndex, float fValue, CLinkedList<GUI_PARAMETER>& guiParameters, bool bLoadingPreset)
{
...
bool __stdcall CDualDelay::processAuxInputBus(audioProcessData* pAudioProcessData)
{
...
*/
bool __stdcall CDualDelay::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
/* processAudioBuffer
	...
	******************************
	...
	m_HostProcessInfo.uTimeSigDenomintor = Host Time Signature Denominator (if supported by host)
*/
bool __stdcall CDualDelay::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CDualDelay::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
bool __stdcall CDualDelay::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CDualDelay::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CDualDelay::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CDualDelay::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CDualDelay::midiClock()
{
...
bool __stdcall CDualDelay::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
*/
void CDualDelay::doVSTSampleAccurateParamUpdates()
{
...
void* __stdcall CDualDelay::showGUI(void* pInfo)
{
...
bool __stdcall CDualDelay::initUI()
{
	// ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! -------------------------------------- //
...
// **--0xDEA7--**
	...
	ui1->fUserDisplayDataHiLimit = 2000.000000;
	...
	ui1->m_pUserCookedFloatData = &m_fDelayTwo;
	ui1->m_pUserCookedDoubleData = NULL;
	ui1->m_pUserCookedUINTData = NULL;
	ui1->cControlUnits = "ms";
	ui1->cVariableName = "m_fDelayTwo";
	...
	ui2->m_pUserCookedFloatData = &m_fWetLevel;
	ui2->m_pUserCookedDoubleData = NULL;
	ui2->m_pUserCookedUINTData = NULL;
	ui2->cControlUnits = "%";
	ui2->cVariableName = "m_fWetLevel";
	...
	ui3->m_pUserCookedFloatData = &m_f_Feedback;
	ui3->m_pUserCookedDoubleData = NULL;
	ui3->m_pUserCookedUINTData = NULL;
	ui3->cControlUnits = "%";
	ui3->cVariableName = "m_f_Feedback";
	...
	ui4->fUserDisplayDataHiLimit = 20500.000000;
	ui4->uUserDataType = floatData;
	ui4->fInitUserIntValue = 0;
	ui4->fInitUserFloatValue = 20.000000;
	ui4->fInitUserDoubleValue = 0;
	ui4->fInitUserUINTValue = 0;
	ui4->m_pUserCookedIntData = NULL;
	ui4->m_pUserCookedFloatData = &m_f_fc_HPF;
	ui4->m_pUserCookedDoubleData = NULL;
	ui4->m_pUserCookedUINTData = NULL;
	ui4->cControlUnits = "Hz";
	ui4->cVariableName = "m_f_fc_HPF";
	ui4->cEnumeratedList = "SEL1,SEL2,SEL3";
	ui4->dPresetData[0] = 20.000000;ui4->dPresetData[1] = 0.000000;ui4->dPresetData[2] = 0.000000;ui4->dPresetData[3] = 0.000000;ui4->dPresetData[4] = 0.000000;ui4->dPresetData[5] = 0.000000;ui4->dPresetData[6] = 0.000000;ui4->dPresetData[7] = 0.000000;ui4->dPresetData[8] = 0.000000;ui4->dPresetData[9] = 0.000000;ui4->dPresetData[10] = 0.000000;ui4->dPresetData[11] = 0.000000;ui4->dPresetData[12] = 0.000000;ui4->dPresetData[13] = 0.000000;ui4->dPresetData[14] = 0.000000;ui4->dPresetData[15] = 0.000000;
	...
	ui6->fUserDisplayDataHiLimit = 0.000000;
	ui6->uUserDataType = floatData;
	ui6->fInitUserIntValue = 0;
	ui6->fInitUserFloatValue = -6.000000;
	ui6->fInitUserDoubleValue = 0;
	ui6->fInitUserUINTValue = 0;
	ui6->m_pUserCookedIntData = NULL;
	ui6->m_pUserCookedFloatData = &m_fVolume_dB;
	ui6->m_pUserCookedDoubleData = NULL;
	ui6->m_pUserCookedUINTData = NULL;
	ui6->cControlUnits = "dB";
	ui6->cVariableName = "m_fVolume_dB";
	ui6->cEnumeratedList = "SEL1,SEL2,SEL3";
	ui6->dPresetData[0] = -6.000000;ui6->dPresetData[1] = 0.000000;ui6->dPresetData[2] = 0.000000;ui6->dPresetData[3] = 0.000000;ui6->dPresetData[4] = 0.000000;ui6->dPresetData[5] = 0.000000;ui6->dPresetData[6] = 0.000000;ui6->dPresetData[7] = 0.000000;ui6->dPresetData[8] = 0.000000;ui6->dPresetData[9] = 0.000000;ui6->dPresetData[10] = 0.000000;ui6->dPresetData[11] = 0.000000;ui6->dPresetData[12] = 0.000000;ui6->dPresetData[13] = 0.000000;ui6->dPresetData[14] = 0.000000;ui6->dPresetData[15] = 0.000000;
	ui6->cControlName = "Volume";
