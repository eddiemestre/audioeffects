...
// Purpose: calcuate's the bi-quad coefficients which will be shared with the left and right
// objects
// Parameters: float for Cutoff Frequency, float for Q
// Returns: Nothing
...
bool __stdcall CResonantLPF::prepareForPlay()
{
	...
	// clock the raw variables > calculate the initial values
...
{
	// because if either slider is moved we have to recalculate all the coefficients > they are interdependent
	// on both Fc and Q, we can get rid of switch statement that checks the slider nControlIndex values
