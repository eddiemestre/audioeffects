...
//
// Function: resetDelay()
// Purpose: flushes and resets the buffer
// Paramters: nothing
// Returns: nothing
void CDDLModule::resetDelay() {

	// flush buffer
	// uses the memset function to flush the buffer of data
	if (m_pBuffer) {
...
// cookVariables()
//
// Function: cookVariables
// Purpose: figures out the delat time in samples from the delay time in milliseconds
// Parameters: none
// Returns: nothing
void CDDLModule::cookVariables() 
{
	// Convert feedback and wet levels
	m_fFeedback = m_f_Feedback_pct / 100.0;
	m_fWetLevel = m_f_WetLevel_pct / 100.0;

	// set DelayInSamples using cooked functions
	m_fDelayInSamples = m_fDelay_ms * ((float)m_nSampleRate / 1000.0);

	// add the code for updating the read index, set the read and write indices depending
	// on the amount of delay time
	m_nReadIndex = m_nWriteIndex - (int)m_fDelayInSamples;		// cast as int!

	// check and wrap BACKWARDS if the index is negative
	if (m_nReadIndex < 0) {
		m_nReadIndex += m_nBufferSize;		// amount of wrap is Read + Length as in equation
	}
}
...
bool __stdcall CDDLModule::prepareForPlay()
{
	...
	// create the new buffer of floats
...
bool __stdcall CDDLModule::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	// call setUsesExternalFeedback function
	setUsesExternalFeedback(false);

	// write the input to the delay
	if (m_bUseExternalFeedback == false) {
		m_pBuffer[m_nWriteIndex] = xn + m_fFeedback * yn; // normal
	}
	else {
		// if user is using external feedback, calls setCurrentFeedbackInput
		setCurrentFeedbackInput(10.0);
	...
	// DLL Module is MONO - just do a copy here too
